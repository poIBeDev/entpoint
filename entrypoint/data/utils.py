...
def get_moex_index():
    try:
        ...
        if marketdata_data:
            ...
            logger.info(f"MOEX данные: current_value={current_value}, moex_date={moex_date}, day_change={day_change}, day_change_prc={day_change_prc}")
        return current_value, moex_date, day_change, day_change_prc
    except Exception as e:
        logger.error(f"Ошибка при получении данных MOEX: {e}", exc_info=True)
...
def get_currency_rate(char_code):
    try:
        ...
        print(f"Валюта {char_code} не найдена.")
        return None, None
...
def get_oil_price():
    try:
        ...
        if not data.empty and len(data) >= 2:
            ...
            return current_price, oil_date, day_change, day_change_prc
        else:
